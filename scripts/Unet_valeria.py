"""
This file contains our implementation of the U-net module,
...
class UNet(nn.Module):
    ...
    * `is_attn`: List o booleans, default is (False, True, True)
    ...
    def __init__(self, n_channels: int = 3, n_classes: int = 1, n_conv_filters: int = 32,
        ...
        # Upsample blocks
        self.up1 = ConvBlockUpsample(n_conv_filters * 8, n_conv_filters * 4, dropout=dropout,
                                     time_channels=n_conv_filters * 4, has_attn=is_attn[2])
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        * `t` has shape `[batch_size]`
        ...
        x, up = self.down1(x, up, t)
        x, up = self.down2(x, up, t)
        x, up = self.down3(x, up, t)
